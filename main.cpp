...
void benchmark(Dictionary* dict, const std::string& label, int N, std::ofstream& csv) {
    ...
    auto total_remove_ns = std::chrono::duration_cast<std::chrono::nanoseconds>(end_remove - start_remove).count();

    long long avg_lookup_ns = total_lookup_time_ns / M;
    long long avg_remove_ns = total_remove_ns / M;

    std::cout << label << " [" << "N=" << N << "] — Insert: " << insert_time << "us, Avg Lookup: " << avg_lookup_ns << "ns, Avg Remove: " << avg_remove_ns << "ns\n";

    csv << label << "," << "," << N << "," << insert_time << "," << avg_remove_ns << "\n";
